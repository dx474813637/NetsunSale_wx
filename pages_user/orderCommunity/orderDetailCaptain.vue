	...
	<view class="w u-p-20" >
		...
		<view class="bg-white u-radius-12 u-p-30 u-info-dark u-m-b-30">
			...
			<template v-if="list.address">
				<view class="u-flex u-flex-items-start u-m-b-20 u-flex-between u-font-28">
					<view class="item text-nowrap u-p-r-20">联系人</view>
					<view class="item u-flex-1 u-text-right text-base">{{ list.address.name }}</view>
				</view>
				<view class="u-flex u-flex-items-start u-m-b-20 u-flex-between u-font-28">
					<view class="item text-nowrap u-p-r-20">联系号码</view>
					<view class="item u-flex-1 u-text-right text-base">{{ list.address.tel }}</view>
				</view>
			</template>
		...
		<view class="bg-white u-radius-12 u-flex u-flex-end u-flex-wrap btns-w " style="padding-left: 90px;">
			...
			<view class="item u-p-15" v-if="btnList.button2"> 
				<!-- 收货确认 -->
				<up-button type="error" shape="circle" @click="button2Event">{{btnList.button2_title}}</up-button>
			</view>
		...
		<view class="bg-white u-radius-12 u-flex u-flex-end u-flex-wrap btns-w u-m-t-40 u-m-b-20" style="padding-left: 90px;">
			...
			<view class="item u-p-20" v-if="btnList.button4"> 
				<!-- 我要退款 -->
				<up-button type="error" shape="circle" plain @click="button4Event">{{btnList.button4_title}}</up-button>
			</view>
...
<script setup>
	...
	async function getData() {
		const res = await $api.order_detail2({
			params: {
				order_id: id.value
			}
		})
	...
	function button2Event() {
		uni.showModal({
			...
			success: async function (res) {
				if (res.confirm) {
					const r = await $api.button2_handle({
						params: {
							id: list.value.id
						}
					})
	...
	function button4Event() {
		uni.showModal({
			...
			success: async function (res) { 
				if (res.confirm) {
					const r = await $api.button4_handle({
						params: {
							id: list.value.id
						}
					})
